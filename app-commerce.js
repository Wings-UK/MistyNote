...
async function renderProductPage(productId) {
  ...
  el.innerHTML = `
    <div class="pdp-images" id="pdp-images">
      ...
      ${images.length > 1 ? `<div class="pdp-img-dots">${images.map((_,i)=>`<div class="pdp-img-dot ${i===0?'active':''}" onclick="pdpGoToImage(${i})"></div>`).join('')}</div>` : ''}
    ...
    <div class="pdp-body">
      <div class="pdp-price-row">
        ...
        ${p.compare_price_ngn > p.price_ngn ? `<span class="pdp-compare-price">${mktFmtNgn(p.compare_price_ngn)}</span>` : ''}
        <span class="pdp-mp-price">${fmtPts(mktNgnToMp(p.price_ngn))}</span>
      </div>
      <div class="pdp-title">${escHtml(p.title)}</div>
      ${p.rating > 0 ? `<div class="pdp-rating"><span class="pdp-stars">${'★'.repeat(Math.round(p.rating))}${'☆'.repeat(5-Math.round(p.rating))}</span><span class="pdp-rating-count">${p.rating} (${p.review_count} reviews)</span></div>` : ''}
      <div class="pdp-store-row" onclick="openStorefront('${sf.id}')">
            ...
            <div class="pdp-variant-options">
              ${(v.options||[]).map((opt,i) => `
                <button class="pdp-variant-opt ${i===0?'selected':''} ${opt.stock===0?'out-of-stock':''}"
                  ...
                  ${escHtml(opt.name)}
                  ${opt.price_ngn && opt.price_ngn !== p.price_ngn ? `<span style="font-size:10px;opacity:0.7">${mktFmtNgn(opt.price_ngn)}</span>` : ''}
                </button>`).join('')}
            </div>
          </div>`).join('')}
      </div>` : ''}
      <div class="pdp-qty-row">
        ...
        <div class="pdp-stock-hint">${p.stock > 0 ? p.stock + ' in stock' : 'Out of stock'}</div>
      </div>
      ${p.description ? `<div class="pdp-section"><div class="pdp-section-title">Description</div><div class="pdp-description">${escHtml(p.description)}</div></div>` : ''}
      <div class="pdp-section">
        <div class="pdp-section-title">Details</div>
      ...
      ${reviews.length > 0 ? `
      <div class="pdp-section">
        <div class="pdp-section-title">Reviews (${reviews.length})</div>
        ${reviews.map(r => `
          <div class="sf-review">
            <div class="sf-review-header">
              <img class="sf-review-avatar" src="${r.reviewer?.avatar||''}" alt="">

...
// ══════════════════════════════════════════
// Docs: https://sendbox.co/developers
// Set your live secret key below.
...
// Paste your ACCESS TOKEN from developers.sendbox.co → Application → Authentication
// Regenerate it there any time it expires
...
// Webhook endpoint registered on Sendbox dashboard (already configured)
// const SENDBOX_WEBHOOK_URL = 'https://mistynote.pages.dev/api/sendbox-webhook';

const SENDBOX_BASE = 'https://api.sendbox.co';

// Runtime token — starts as the access token, gets swapped on refresh
let _sendboxToken = SENDBOX_ACCESS_TOKEN;

/**
 * Wrapper for all Sendbox API calls.
...
async function sendboxRequest(method, path, body, _isRetry = false) {
  const opts = {
    ...
    headers: {
      ...
      'Authorization': 'Bearer ' + _sendboxToken,
    },
  };
  ...
  if (res.status === 401 && !_isRetry) {
    const refreshed = await _sendboxRefreshToken();
    if (refreshed) return sendboxRequest(method, path, body, true);
  }
...
async function _sendboxRefreshToken() {
  try {
    const res  = await fetch(SENDBOX_BASE + '/auth/token/refresh', {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({ refresh_token: SENDBOX_REFRESH_TOKEN }),
    });
    const json = await res.json();
    const newToken = json?.data?.access_token || json?.access_token;
    if (newToken) { _sendboxToken = newToken; return true; }
    return false;
  } catch (e) {
    console.error('[Sendbox] Token refresh failed:', e);
    return false;
  }
}
...
async function loadShippingRates() {
  ...
  for (const sfId of storeIds) {
    ...
    // Get seller's pickup address from storefront
    const { data: sf } = await supabase
      .from('storefronts')
      .select('pickup_state, pickup_address')
      .eq('id', sfId)
      .maybeSingle();
    ...
    try {
      ...
      // Build Sendbox rate request
      // origin = seller's state; destination = buyer's selected state
      const rateRes = await sendboxRequest('POST', '/shipping/rates', {
        origin: {
          state:   sf?.pickup_state   || 'Lagos',
          address: sf?.pickup_address || '',
          country: 'NG',
        },
        destination: {
          state:   state,
          address: streetEl?.value?.trim() || '',
          country: 'NG',
        },
        weight:  1,   // default 1 kg — update per product if you track weight
        length:  10,
        width:   10,
        height:  10,
        type:    'parcel',
      ...
      // Pick cheapest available rate from Sendbox response
      const rates = rateRes?.data || rateRes?.rates || [];

      if (rates.length > 0) {

        const cheapest = rates.reduce((a, b) => (a.amount < b.amount ? a : b));
        ...
        // Sendbox returned no rates — fall back to your DB table
...
async function openShipOrder(orderId) {
  ...
  // Fetch order + storefront for pickup address
  const { data: order } = await supabase
    .from('orders')
    .select('*, storefronts(pickup_state, pickup_address, pickup_name, pickup_phone)')
    .eq('id', orderId)
    .single();
  ...
  sheet.innerHTML = `
    <div style="width:100%;background:var(--surface);border-radius:24px 24px 0 0;padding:24px 20px calc(var(--safe-bottom)+24px)">
      ...
      <div style="font-size:13px;color:var(--text3);margin-bottom:20px">Sendbox will book a pickup and generate a waybill automatically</div>
  ...
  // Attach order + address data to the sheet element for submitShipOrder to use
...
async function submitShipOrder(orderId, sheetEl) {
  ...
  // Pull pre-attached order data from the sheet
  ...
  try {
    ...
    // ── Call Sendbox: create shipment ──────────────────────
    const shipmentRes = await sendboxRequest('POST', '/shipping/shipments', {
      sender: {
        name:    sf.pickup_name    || currentProfile?.username || '',
        phone:   sf.pickup_phone   || currentProfile?.phone    || '',
        address: sf.pickup_address || '',
        state:   sf.pickup_state   || 'Lagos',
        country: 'NG',
      },
      ...
      parcel: {
        ...
        width:       10,
        height:      10,
        description: order.title || 'Order',
        value:       order.price_ngn || 0,
      },
      note:            noteText,
      delivery_type:   'door_delivery',
      payment_method:  'prepaid',  // Platform pays Sendbox; charge already captured via Flutterwave
    });
    ...
    await supabase.from('orders').update({
      ...
      sendbox_tracking_url: trackingUrl,
      sendbox_status:      'booked',
    ...
    // ── Graceful fallback: let seller enter manually ───────
    showActionSheet([
      {
        label: '📷 Upload Proof Manually Instead',
        action: () => {
          sheetEl?.remove();
          _fallbackManualShip(orderId);
        },
      },
...
/** Fallback: manual courier + photo upload (original flow) */
...
async function _fallbackSubmitManualShip(orderId, sheetEl) {
  ...
  input.onchange = async (e) => {
    ...
    try {
      ...
      if (order) insertNotification({
        user_id: order.buyer_id, actor_id: currentUser.id, type: 'order_shipped',
...
async function openEditStorefront() {
  ...
  // Fetch latest storefront data including pickup fields
  const { data: sf } = await supabase
    .from('storefronts')
  ...
  const stateOptions = nigeriaStates.map(s =>
    `<option value="${s}" ${sf.pickup_state === s ? 'selected' : ''}>${s}</option>`
  ).join('');
  ...
  overlay.innerHTML =
    ...
    '<div style="padding:16px;display:flex;flex-direction:column;gap:12px">' +
      ...
      '<div style="background:var(--surface);border-radius:16px;padding:16px;border:1px solid var(--border)">' +
        ...
        '<div style="margin-bottom:12px">' +
          '<div style="font-size:12px;font-weight:600;color:var(--text3);margin-bottom:6px">Store Name</div>' +
          '<input id="esf-name" class="co-input" value="' + escHtml(sf.store_name || '') + '" placeholder="Your store name" style="width:100%;box-sizing:border-box">' +
        '</div>' +

        '<div style="margin-bottom:12px">' +
          '<div style="font-size:12px;font-weight:600;color:var(--text3);margin-bottom:6px">Category</div>' +
          '<input id="esf-category" class="co-input" value="' + escHtml(sf.category || '') + '" placeholder="e.g. Fashion, Electronics, Food…" style="width:100%;box-sizing:border-box">' +
        '</div>' +

        '<div>' +
          '<div style="font-size:12px;font-weight:600;color:var(--text3);margin-bottom:6px">Description</div>' +
          '<textarea id="esf-description" class="co-input" rows="3" placeholder="Tell buyers about your store…" style="width:100%;box-sizing:border-box;resize:none;min-height:72px">' + escHtml(sf.description || '') + '</textarea>' +
        '</div>' +
      '</div>' +

      // ── Pickup Address (Sendbox) ──
      '<div style="background:var(--surface);border-radius:16px;padding:16px;border:1px solid var(--border)">' +
        ...
        '<div style="font-size:12px;color:var(--text3);margin-bottom:14px;line-height:1.5">Where Sendbox will collect orders from. Must be accurate for couriers to find you.</div>' +

        '<div style="margin-bottom:12px">' +
          '<div style="font-size:12px;font-weight:600;color:var(--text3);margin-bottom:6px">Contact Name</div>' +
          '<input id="esf-pickup-name" class="co-input" value="' + escHtml(sf.pickup_name || '') + '" placeholder="Full name of person at pickup" style="width:100%;box-sizing:border-box">' +
        '</div>' +

        '<div style="margin-bottom:12px">' +
          '<div style="font-size:12px;font-weight:600;color:var(--text3);margin-bottom:6px">Contact Phone</div>' +
          '<input id="esf-pickup-phone" class="co-input" type="tel" value="' + escHtml(sf.pickup_phone || '') + '" placeholder="e.g. 08012345678" style="width:100%;box-sizing:border-box">' +
        '</div>' +

        '<div style="margin-bottom:12px">' +
          '<div style="font-size:12px;font-weight:600;color:var(--text3);margin-bottom:6px">State</div>' +
          '<select id="esf-pickup-state" class="co-input" style="width:100%;box-sizing:border-box">' +
        ...
        '<div>' +
          '<div style="font-size:12px;font-weight:600;color:var(--text3);margin-bottom:6px">Street Address</div>' +
          '<input id="esf-pickup-address" class="co-input" value="' + escHtml(sf.pickup_address || '') + '" placeholder="House/flat number, street, area" style="width:100%;box-sizing:border-box">' +
        '</div>' +
      '</div>' +
...
async function saveEditStorefront() {
  ...
  try {
    ...
    // Refresh local storefront state

/* ═══════════════════════════════════════════
   ...
   MistyPoints wallet, Flutterwave payment, CheapData
...
// Flutterwave handles all payment gateway operations.
...
const FLW_PUBLIC_KEY = 'FLWPUBK-7bcaf5697d2ee4a2a4437f3ec59cbe22-X';
...
function initiateBuyPoints() {
  ...
  // Load Flutterwave inline script once then run payment

  if (typeof FlutterwaveCheckout === 'undefined') {

    loadScript('https://checkout.flutterwave.com/v3.js', function() {

      runFlutterwavePayment(points, ngnAmount);
...
function runFlutterwavePayment(points, ngnAmount) {
  ...
  FlutterwaveCheckout({
    ...
    amount:          Math.round(ngnAmount),  // NGN whole number — Flutterwave does not use kobo
    ...
    customer: {
      ...
      name:  (currentUser.user_metadata && currentUser.user_metadata.full_name) || currentUser.email,
    ...
    customizations: {
      ...
      description: 'Buy MistyPoints',

      logo:        'https://mistynote.pages.dev/icon-192.png',
    ...
    callback: async function(response) {
      ...
      if (isSuccess) {
        ...
        try {
          ...
          await creditPointsFlutterwave(txRef, points);
...
async function creditPointsFlutterwave(txRef, points) {

  // Calls Supabase Edge Function which verifies with Flutterwave server-side

  // then credits the wallet via RPC — prevents any client-side manipulation

  var res = await supabase.functions.invoke('credit-points', {

    body: { tx_ref: txRef, points: points, user_id: currentUser.id, gateway: 'flutterwave' }
...
// Kept for legacy references — no-ops
